; NOTE: Assertions have been autogenerated by test/update_tpde_llc_test_checks.py UTC_ARGS: --version 5
; SPDX-FileCopyrightText: 2025 Contributors to TPDE <https://tpde.org>
; SPDX-License-Identifier: Apache-2.0 WITH LLVM-exception

; RUN: tpde-llc --target=x86_64 %s | %objdump | FileCheck %s -check-prefixes=X64
; RUN: tpde-llc --target=aarch64 %s | %objdump | FileCheck %s -check-prefixes=ARM64

define i16 @sext_i8_to_i16(i8 %0) {
; X64-LABEL: <sext_i8_to_i16>:
; X64:         movsx edi, dil
; X64-NEXT:    mov eax, edi
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i8_to_i16>:
; ARM64:         sxtb w0, w0
; ARM64-NEXT:    ret
entry:
  %1 = sext i8 %0 to i16
  ret i16 %1
}

define i21 @sext_i8_to_i21(i8 %0) {
; X64-LABEL: <sext_i8_to_i21>:
; X64:         movsx edi, dil
; X64-NEXT:    mov eax, edi
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i8_to_i21>:
; ARM64:         sxtb w0, w0
; ARM64-NEXT:    ret
entry:
  %1 = sext i8 %0 to i21
  ret i21 %1
}

define i32 @sext_i8_to_i32(i8 %0) {
; X64-LABEL: <sext_i8_to_i32>:
; X64:         movsx edi, dil
; X64-NEXT:    mov eax, edi
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i8_to_i32>:
; ARM64:         sxtb w0, w0
; ARM64-NEXT:    ret
entry:
  %1 = sext i8 %0 to i32
  ret i32 %1
}

define i37 @sext_i8_to_i37(i8 %0) {
; X64-LABEL: <sext_i8_to_i37>:
; X64:         movsx rdi, dil
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i8_to_i37>:
; ARM64:         sxtb x0, w0
; ARM64-NEXT:    ret
entry:
  %1 = sext i8 %0 to i37
  ret i37 %1
}

define i64 @sext_i8_to_i64(i8 %0) {
; X64-LABEL: <sext_i8_to_i64>:
; X64:         movsx rdi, dil
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i8_to_i64>:
; ARM64:         sxtb x0, w0
; ARM64-NEXT:    ret
entry:
  %1 = sext i8 %0 to i64
  ret i64 %1
}

define i80 @sext_i8_to_i80(i8 %0) {
; X64-LABEL: <sext_i8_to_i80>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    movsx rdi, dil
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    sar rax, 0x3f
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i8_to_i80>:
; ARM64:         sxtb x0, w0
; ARM64-NEXT:    asr x1, x0, #63
; ARM64-NEXT:    ret
entry:
  %1 = sext i8 %0 to i80
  ret i80 %1
}

define i128 @sext_i8_to_i128(i8 %0) {
; X64-LABEL: <sext_i8_to_i128>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    movsx rdi, dil
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    sar rax, 0x3f
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i8_to_i128>:
; ARM64:         sxtb x0, w0
; ARM64-NEXT:    asr x1, x0, #63
; ARM64-NEXT:    ret
entry:
  %1 = sext i8 %0 to i128
  ret i128 %1
}


define i21 @sext_i16_to_i21(i16 %0) {
; X64-LABEL: <sext_i16_to_i21>:
; X64:         movsx edi, di
; X64-NEXT:    mov eax, edi
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i16_to_i21>:
; ARM64:         sxth w0, w0
; ARM64-NEXT:    ret
entry:
  %1 = sext i16 %0 to i21
  ret i21 %1
}

define i32 @sext_i16_to_i32(i16 %0) {
; X64-LABEL: <sext_i16_to_i32>:
; X64:         movsx edi, di
; X64-NEXT:    mov eax, edi
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i16_to_i32>:
; ARM64:         sxth w0, w0
; ARM64-NEXT:    ret
entry:
  %1 = sext i16 %0 to i32
  ret i32 %1
}

define i37 @sext_i16_to_i37(i16 %0) {
; X64-LABEL: <sext_i16_to_i37>:
; X64:         movsx rdi, di
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i16_to_i37>:
; ARM64:         sxth x0, w0
; ARM64-NEXT:    ret
entry:
  %1 = sext i16 %0 to i37
  ret i37 %1
}

define i64 @sext_i16_to_i64(i16 %0) {
; X64-LABEL: <sext_i16_to_i64>:
; X64:         movsx rdi, di
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i16_to_i64>:
; ARM64:         sxth x0, w0
; ARM64-NEXT:    ret
entry:
  %1 = sext i16 %0 to i64
  ret i64 %1
}

define i80 @sext_i16_to_i80(i16 %0) {
; X64-LABEL: <sext_i16_to_i80>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    movsx rdi, di
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    sar rax, 0x3f
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i16_to_i80>:
; ARM64:         sxth x0, w0
; ARM64-NEXT:    asr x1, x0, #63
; ARM64-NEXT:    ret
entry:
  %1 = sext i16 %0 to i80
  ret i80 %1
}

define i128 @sext_i16_to_i128(i16 %0) {
; X64-LABEL: <sext_i16_to_i128>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    movsx rdi, di
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    sar rax, 0x3f
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i16_to_i128>:
; ARM64:         sxth x0, w0
; ARM64-NEXT:    asr x1, x0, #63
; ARM64-NEXT:    ret
entry:
  %1 = sext i16 %0 to i128
  ret i128 %1
}


define i32 @sext_i21_to_i32(i21 %0) {
; X64-LABEL: <sext_i21_to_i32>:
; X64:         shl edi, 0xb
; X64-NEXT:    sar edi, 0xb
; X64-NEXT:    mov eax, edi
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i21_to_i32>:
; ARM64:         sbfx w0, w0, #0, #21
; ARM64-NEXT:    ret
entry:
  %1 = sext i21 %0 to i32
  ret i32 %1
}

define i37 @sext_i21_to_i37(i21 %0) {
; X64-LABEL: <sext_i21_to_i37>:
; X64:         shl rdi, 0x2b
; X64-NEXT:    sar rdi, 0x2b
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i21_to_i37>:
; ARM64:         sbfx x0, x0, #0, #21
; ARM64-NEXT:    ret
entry:
  %1 = sext i21 %0 to i37
  ret i37 %1
}

define i64 @sext_i21_to_i64(i21 %0) {
; X64-LABEL: <sext_i21_to_i64>:
; X64:         shl rdi, 0x2b
; X64-NEXT:    sar rdi, 0x2b
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i21_to_i64>:
; ARM64:         sbfx x0, x0, #0, #21
; ARM64-NEXT:    ret
entry:
  %1 = sext i21 %0 to i64
  ret i64 %1
}

define i80 @sext_i21_to_i80(i21 %0) {
; X64-LABEL: <sext_i21_to_i80>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    shl rdi, 0x2b
; X64-NEXT:    sar rdi, 0x2b
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    sar rax, 0x3f
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i21_to_i80>:
; ARM64:         sbfx x0, x0, #0, #21
; ARM64-NEXT:    asr x1, x0, #63
; ARM64-NEXT:    ret
entry:
  %1 = sext i21 %0 to i80
  ret i80 %1
}

define i128 @sext_i21_to_i128(i21 %0) {
; X64-LABEL: <sext_i21_to_i128>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    shl rdi, 0x2b
; X64-NEXT:    sar rdi, 0x2b
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    sar rax, 0x3f
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i21_to_i128>:
; ARM64:         sbfx x0, x0, #0, #21
; ARM64-NEXT:    asr x1, x0, #63
; ARM64-NEXT:    ret
entry:
  %1 = sext i21 %0 to i128
  ret i128 %1
}


define i37 @sext_i32_to_i37(i32 %0) {
; X64-LABEL: <sext_i32_to_i37>:
; X64:         movsxd rdi, edi
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i32_to_i37>:
; ARM64:         sxtw x0, w0
; ARM64-NEXT:    ret
entry:
  %1 = sext i32 %0 to i37
  ret i37 %1
}

define i64 @sext_i32_to_i64(i32 %0) {
; X64-LABEL: <sext_i32_to_i64>:
; X64:         movsxd rdi, edi
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i32_to_i64>:
; ARM64:         sxtw x0, w0
; ARM64-NEXT:    ret
entry:
  %1 = sext i32 %0 to i64
  ret i64 %1
}

define i80 @sext_i32_to_i80(i32 %0) {
; X64-LABEL: <sext_i32_to_i80>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    movsxd rdi, edi
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    sar rax, 0x3f
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i32_to_i80>:
; ARM64:         sxtw x0, w0
; ARM64-NEXT:    asr x1, x0, #63
; ARM64-NEXT:    ret
entry:
  %1 = sext i32 %0 to i80
  ret i80 %1
}

define i128 @sext_i32_to_i128(i32 %0) {
; X64-LABEL: <sext_i32_to_i128>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    movsxd rdi, edi
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    sar rax, 0x3f
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i32_to_i128>:
; ARM64:         sxtw x0, w0
; ARM64-NEXT:    asr x1, x0, #63
; ARM64-NEXT:    ret
entry:
  %1 = sext i32 %0 to i128
  ret i128 %1
}


define i64 @sext_i37_to_i64(i37 %0) {
; X64-LABEL: <sext_i37_to_i64>:
; X64:         shl rdi, 0x1b
; X64-NEXT:    sar rdi, 0x1b
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i37_to_i64>:
; ARM64:         sbfx x0, x0, #0, #37
; ARM64-NEXT:    ret
entry:
  %1 = sext i37 %0 to i64
  ret i64 %1
}

define i80 @sext_i37_to_i80(i37 %0) {
; X64-LABEL: <sext_i37_to_i80>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    shl rdi, 0x1b
; X64-NEXT:    sar rdi, 0x1b
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    sar rax, 0x3f
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i37_to_i80>:
; ARM64:         sbfx x0, x0, #0, #37
; ARM64-NEXT:    asr x1, x0, #63
; ARM64-NEXT:    ret
entry:
  %1 = sext i37 %0 to i80
  ret i80 %1
}

define i128 @sext_i37_to_i128(i37 %0) {
; X64-LABEL: <sext_i37_to_i128>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    shl rdi, 0x1b
; X64-NEXT:    sar rdi, 0x1b
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    sar rax, 0x3f
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i37_to_i128>:
; ARM64:         sbfx x0, x0, #0, #37
; ARM64-NEXT:    asr x1, x0, #63
; ARM64-NEXT:    ret
entry:
  %1 = sext i37 %0 to i128
  ret i128 %1
}


define i80 @sext_i64_to_i80(i64 %0) {
; X64-LABEL: <sext_i64_to_i80>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    sar rax, 0x3f
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i64_to_i80>:
; ARM64:         asr x1, x0, #63
; ARM64-NEXT:    ret
entry:
  %1 = sext i64 %0 to i80
  ret i80 %1
}

define i80 @sext_i64_to_i80_noreuse(i64 %i, ptr %p) {
; X64-LABEL: <sext_i64_to_i80_noreuse>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    sar rax, 0x3f
; X64-NEXT:    mov rcx, rdi
; X64-NEXT:    mov qword ptr [rsi], rdi
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rcx
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i64_to_i80_noreuse>:
; ARM64:         asr x2, x0, #63
; ARM64-NEXT:    mov x3, x0
; ARM64-NEXT:    str x0, [x1]
; ARM64-NEXT:    mov x0, x3
; ARM64-NEXT:    mov x1, x2
; ARM64-NEXT:    ret
  %e = sext i64 %i to i80
  store i64 %i, ptr %p
  ret i80 %e
}

define i128 @sext_i64_to_i128(i64 %0) {
; X64-LABEL: <sext_i64_to_i128>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    sar rax, 0x3f
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i64_to_i128>:
; ARM64:         asr x1, x0, #63
; ARM64-NEXT:    ret
entry:
  %1 = sext i64 %0 to i128
  ret i128 %1
}

define i128 @sext_i64_to_i128_noreuse(i64 %i, ptr %p) {
; X64-LABEL: <sext_i64_to_i128_noreuse>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    sar rax, 0x3f
; X64-NEXT:    mov rcx, rdi
; X64-NEXT:    mov qword ptr [rsi], rdi
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rcx
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i64_to_i128_noreuse>:
; ARM64:         asr x2, x0, #63
; ARM64-NEXT:    mov x3, x0
; ARM64-NEXT:    str x0, [x1]
; ARM64-NEXT:    mov x0, x3
; ARM64-NEXT:    mov x1, x2
; ARM64-NEXT:    ret
  %e = sext i64 %i to i128
  store i64 %i, ptr %p
  ret i128 %e
}


define i128 @sext_i80_to_i128(i80 %0) {
; X64-LABEL: <sext_i80_to_i128>:
; X64:         movsx rsi, si
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, rsi
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i80_to_i128>:
; ARM64:         sxth x1, w1
; ARM64-NEXT:    ret
entry:
  %1 = sext i80 %0 to i128
  ret i128 %1
}

define i128 @sext_i80_to_i128_noreuse(i80 %0) {
; X64-LABEL: <sext_i80_to_i128_noreuse>:
; X64:         mov rax, rdi
; X64-NEXT:    movsx rcx, si
; X64-NEXT:    movsx rsi, si
; X64-NEXT:    mov rdx, rcx
; X64-NEXT:    ret
;
; ARM64-LABEL: <sext_i80_to_i128_noreuse>:
; ARM64:         mov x2, x0
; ARM64-NEXT:    sxth x3, w1
; ARM64-NEXT:    sxth x1, w1
; ARM64-NEXT:    mov x0, x2
; ARM64-NEXT:    mov x1, x3
; ARM64-NEXT:    ret
entry:
  %1 = sext i80 %0 to i128
  %2 = sext i80 %0 to i128
  ret i128 %1
}
