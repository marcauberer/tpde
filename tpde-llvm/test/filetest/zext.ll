; NOTE: Assertions have been autogenerated by test/update_tpde_llc_test_checks.py UTC_ARGS: --version 5
; SPDX-FileCopyrightText: 2025 Contributors to TPDE <https://tpde.org>
; SPDX-License-Identifier: Apache-2.0 WITH LLVM-exception

; RUN: tpde-llc --target=x86_64 %s | %objdump | FileCheck %s -check-prefixes=X64
; RUN: tpde-llc --target=aarch64 %s | %objdump | FileCheck %s -check-prefixes=ARM64

define i16 @zext_i8_to_i16(i8 %0) {
; X64-LABEL: <zext_i8_to_i16>:
; X64:         movzx edi, dil
; X64-NEXT:    mov eax, edi
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i8_to_i16>:
; ARM64:         uxtb w0, w0
; ARM64-NEXT:    ret
entry:
  %1 = zext i8 %0 to i16
  ret i16 %1
}

define i21 @zext_i8_to_i21(i8 %0) {
; X64-LABEL: <zext_i8_to_i21>:
; X64:         movzx edi, dil
; X64-NEXT:    mov eax, edi
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i8_to_i21>:
; ARM64:         uxtb w0, w0
; ARM64-NEXT:    ret
entry:
  %1 = zext i8 %0 to i21
  ret i21 %1
}

define i32 @zext_i8_to_i32(i8 %0) {
; X64-LABEL: <zext_i8_to_i32>:
; X64:         movzx edi, dil
; X64-NEXT:    mov eax, edi
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i8_to_i32>:
; ARM64:         uxtb w0, w0
; ARM64-NEXT:    ret
entry:
  %1 = zext i8 %0 to i32
  ret i32 %1
}

define i37 @zext_i8_to_i37(i8 %0) {
; X64-LABEL: <zext_i8_to_i37>:
; X64:         movzx edi, dil
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i8_to_i37>:
; ARM64:         ubfx x0, x0, #0, #8
; ARM64-NEXT:    ret
entry:
  %1 = zext i8 %0 to i37
  ret i37 %1
}

define i64 @zext_i8_to_i64(i8 %0) {
; X64-LABEL: <zext_i8_to_i64>:
; X64:         movzx edi, dil
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i8_to_i64>:
; ARM64:         ubfx x0, x0, #0, #8
; ARM64-NEXT:    ret
entry:
  %1 = zext i8 %0 to i64
  ret i64 %1
}

define i80 @zext_i8_to_i80(i8 %0) {
; X64-LABEL: <zext_i8_to_i80>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    movzx edi, dil
; X64-NEXT:    xor eax, eax
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i8_to_i80>:
; ARM64:         ubfx x0, x0, #0, #8
; ARM64-NEXT:    mov w1, #0x0 // =0
; ARM64-NEXT:    ret
entry:
  %1 = zext i8 %0 to i80
  ret i80 %1
}

define i128 @zext_i8_to_i128(i8 %0) {
; X64-LABEL: <zext_i8_to_i128>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    movzx edi, dil
; X64-NEXT:    xor eax, eax
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i8_to_i128>:
; ARM64:         ubfx x0, x0, #0, #8
; ARM64-NEXT:    mov w1, #0x0 // =0
; ARM64-NEXT:    ret
entry:
  %1 = zext i8 %0 to i128
  ret i128 %1
}


define i21 @zext_i16_to_i21(i16 %0) {
; X64-LABEL: <zext_i16_to_i21>:
; X64:         movzx edi, di
; X64-NEXT:    mov eax, edi
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i16_to_i21>:
; ARM64:         uxth w0, w0
; ARM64-NEXT:    ret
entry:
  %1 = zext i16 %0 to i21
  ret i21 %1
}

define i32 @zext_i16_to_i32(i16 %0) {
; X64-LABEL: <zext_i16_to_i32>:
; X64:         movzx edi, di
; X64-NEXT:    mov eax, edi
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i16_to_i32>:
; ARM64:         uxth w0, w0
; ARM64-NEXT:    ret
entry:
  %1 = zext i16 %0 to i32
  ret i32 %1
}

define i37 @zext_i16_to_i37(i16 %0) {
; X64-LABEL: <zext_i16_to_i37>:
; X64:         movzx edi, di
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i16_to_i37>:
; ARM64:         ubfx x0, x0, #0, #16
; ARM64-NEXT:    ret
entry:
  %1 = zext i16 %0 to i37
  ret i37 %1
}

define i64 @zext_i16_to_i64(i16 %0) {
; X64-LABEL: <zext_i16_to_i64>:
; X64:         movzx edi, di
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i16_to_i64>:
; ARM64:         ubfx x0, x0, #0, #16
; ARM64-NEXT:    ret
entry:
  %1 = zext i16 %0 to i64
  ret i64 %1
}

define i80 @zext_i16_to_i80(i16 %0) {
; X64-LABEL: <zext_i16_to_i80>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    movzx edi, di
; X64-NEXT:    xor eax, eax
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i16_to_i80>:
; ARM64:         ubfx x0, x0, #0, #16
; ARM64-NEXT:    mov w1, #0x0 // =0
; ARM64-NEXT:    ret
entry:
  %1 = zext i16 %0 to i80
  ret i80 %1
}

define i128 @zext_i16_to_i128(i16 %0) {
; X64-LABEL: <zext_i16_to_i128>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    movzx edi, di
; X64-NEXT:    xor eax, eax
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i16_to_i128>:
; ARM64:         ubfx x0, x0, #0, #16
; ARM64-NEXT:    mov w1, #0x0 // =0
; ARM64-NEXT:    ret
entry:
  %1 = zext i16 %0 to i128
  ret i128 %1
}


define i32 @zext_i21_to_i32(i21 %0) {
; X64-LABEL: <zext_i21_to_i32>:
; X64:         and edi, 0x1fffff
; X64-NEXT:    mov eax, edi
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i21_to_i32>:
; ARM64:         ubfx w0, w0, #0, #21
; ARM64-NEXT:    ret
entry:
  %1 = zext i21 %0 to i32
  ret i32 %1
}

define i37 @zext_i21_to_i37(i21 %0) {
; X64-LABEL: <zext_i21_to_i37>:
; X64:         and edi, 0x1fffff
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i21_to_i37>:
; ARM64:         ubfx x0, x0, #0, #21
; ARM64-NEXT:    ret
entry:
  %1 = zext i21 %0 to i37
  ret i37 %1
}

define i64 @zext_i21_to_i64(i21 %0) {
; X64-LABEL: <zext_i21_to_i64>:
; X64:         and edi, 0x1fffff
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i21_to_i64>:
; ARM64:         ubfx x0, x0, #0, #21
; ARM64-NEXT:    ret
entry:
  %1 = zext i21 %0 to i64
  ret i64 %1
}

define i80 @zext_i21_to_i80(i21 %0) {
; X64-LABEL: <zext_i21_to_i80>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    and edi, 0x1fffff
; X64-NEXT:    xor eax, eax
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i21_to_i80>:
; ARM64:         ubfx x0, x0, #0, #21
; ARM64-NEXT:    mov w1, #0x0 // =0
; ARM64-NEXT:    ret
entry:
  %1 = zext i21 %0 to i80
  ret i80 %1
}

define i128 @zext_i21_to_i128(i21 %0) {
; X64-LABEL: <zext_i21_to_i128>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    and edi, 0x1fffff
; X64-NEXT:    xor eax, eax
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i21_to_i128>:
; ARM64:         ubfx x0, x0, #0, #21
; ARM64-NEXT:    mov w1, #0x0 // =0
; ARM64-NEXT:    ret
entry:
  %1 = zext i21 %0 to i128
  ret i128 %1
}


define i37 @zext_i32_to_i37(i32 %0) {
; X64-LABEL: <zext_i32_to_i37>:
; X64:         mov edi, edi
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i32_to_i37>:
; ARM64:         ubfx x0, x0, #0, #32
; ARM64-NEXT:    ret
entry:
  %1 = zext i32 %0 to i37
  ret i37 %1
}

define i64 @zext_i32_to_i64(i32 %0) {
; X64-LABEL: <zext_i32_to_i64>:
; X64:         mov edi, edi
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i32_to_i64>:
; ARM64:         ubfx x0, x0, #0, #32
; ARM64-NEXT:    ret
entry:
  %1 = zext i32 %0 to i64
  ret i64 %1
}

define i80 @zext_i32_to_i80(i32 %0) {
; X64-LABEL: <zext_i32_to_i80>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    mov edi, edi
; X64-NEXT:    xor eax, eax
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i32_to_i80>:
; ARM64:         ubfx x0, x0, #0, #32
; ARM64-NEXT:    mov w1, #0x0 // =0
; ARM64-NEXT:    ret
entry:
  %1 = zext i32 %0 to i80
  ret i80 %1
}

define i128 @zext_i32_to_i128(i32 %0) {
; X64-LABEL: <zext_i32_to_i128>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    mov edi, edi
; X64-NEXT:    xor eax, eax
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i32_to_i128>:
; ARM64:         ubfx x0, x0, #0, #32
; ARM64-NEXT:    mov w1, #0x0 // =0
; ARM64-NEXT:    ret
entry:
  %1 = zext i32 %0 to i128
  ret i128 %1
}


define i64 @zext_i37_to_i64(i37 %0) {
; X64-LABEL: <zext_i37_to_i64>:
; X64:         movabs rax, 0x1fffffffff
; X64-NEXT:    and rdi, rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i37_to_i64>:
; ARM64:         ubfx x0, x0, #0, #37
; ARM64-NEXT:    ret
entry:
  %1 = zext i37 %0 to i64
  ret i64 %1
}

define i80 @zext_i37_to_i80(i37 %0) {
; X64-LABEL: <zext_i37_to_i80>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    movabs rax, 0x1fffffffff
; X64-NEXT:    and rdi, rax
; X64-NEXT:    xor eax, eax
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i37_to_i80>:
; ARM64:         ubfx x0, x0, #0, #37
; ARM64-NEXT:    mov w1, #0x0 // =0
; ARM64-NEXT:    ret
entry:
  %1 = zext i37 %0 to i80
  ret i80 %1
}

define i128 @zext_i37_to_i128(i37 %0) {
; X64-LABEL: <zext_i37_to_i128>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    movabs rax, 0x1fffffffff
; X64-NEXT:    and rdi, rax
; X64-NEXT:    xor eax, eax
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i37_to_i128>:
; ARM64:         ubfx x0, x0, #0, #37
; ARM64-NEXT:    mov w1, #0x0 // =0
; ARM64-NEXT:    ret
entry:
  %1 = zext i37 %0 to i128
  ret i128 %1
}


define i80 @zext_i64_to_i80(i64 %0) {
; X64-LABEL: <zext_i64_to_i80>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    xor eax, eax
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i64_to_i80>:
; ARM64:         mov w1, #0x0 // =0
; ARM64-NEXT:    ret
entry:
  %1 = zext i64 %0 to i80
  ret i80 %1
}

define i80 @zext_i64_to_i80_noreuse(i64 %i, ptr %p) {
; X64-LABEL: <zext_i64_to_i80_noreuse>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    xor eax, eax
; X64-NEXT:    mov rcx, rdi
; X64-NEXT:    mov qword ptr [rsi], rdi
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rcx
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i64_to_i80_noreuse>:
; ARM64:         mov w2, #0x0 // =0
; ARM64-NEXT:    mov x3, x0
; ARM64-NEXT:    str x0, [x1]
; ARM64-NEXT:    mov x0, x3
; ARM64-NEXT:    mov x1, x2
; ARM64-NEXT:    ret
  %e = zext i64 %i to i80
  store i64 %i, ptr %p
  ret i80 %e
}

define i128 @zext_i64_to_i128(i64 %0) {
; X64-LABEL: <zext_i64_to_i128>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    xor eax, eax
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i64_to_i128>:
; ARM64:         mov w1, #0x0 // =0
; ARM64-NEXT:    ret
entry:
  %1 = zext i64 %0 to i128
  ret i128 %1
}

define i128 @zext_i64_to_i128_noreuse(i64 %i, ptr %p) {
; X64-LABEL: <zext_i64_to_i128_noreuse>:
; X64:         push rbp
; X64-NEXT:    mov rbp, rsp
; X64-NEXT:    xor eax, eax
; X64-NEXT:    mov rcx, rdi
; X64-NEXT:    mov qword ptr [rsi], rdi
; X64-NEXT:    mov qword ptr [rbp - 0x38], rax
; X64-NEXT:    mov rax, rcx
; X64-NEXT:    mov rdx, qword ptr [rbp - 0x38]
; X64-NEXT:    pop rbp
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i64_to_i128_noreuse>:
; ARM64:         mov w2, #0x0 // =0
; ARM64-NEXT:    mov x3, x0
; ARM64-NEXT:    str x0, [x1]
; ARM64-NEXT:    mov x0, x3
; ARM64-NEXT:    mov x1, x2
; ARM64-NEXT:    ret
  %e = zext i64 %i to i128
  store i64 %i, ptr %p
  ret i128 %e
}


define i128 @zext_i80_to_i128(i80 %0) {
; X64-LABEL: <zext_i80_to_i128>:
; X64:         movzx esi, si
; X64-NEXT:    mov rax, rdi
; X64-NEXT:    mov rdx, rsi
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i80_to_i128>:
; ARM64:         ubfx x1, x1, #0, #16
; ARM64-NEXT:    ret
entry:
  %1 = zext i80 %0 to i128
  ret i128 %1
}

define i128 @zext_i80_to_i128_noreuse(i80 %0) {
; X64-LABEL: <zext_i80_to_i128_noreuse>:
; X64:         mov rax, rdi
; X64-NEXT:    movzx ecx, si
; X64-NEXT:    movzx esi, si
; X64-NEXT:    mov rdx, rcx
; X64-NEXT:    ret
;
; ARM64-LABEL: <zext_i80_to_i128_noreuse>:
; ARM64:         mov x2, x0
; ARM64-NEXT:    ubfx x3, x1, #0, #16
; ARM64-NEXT:    ubfx x1, x1, #0, #16
; ARM64-NEXT:    mov x0, x2
; ARM64-NEXT:    mov x1, x3
; ARM64-NEXT:    ret
entry:
  %1 = zext i80 %0 to i128
  %2 = zext i80 %0 to i128
  ret i128 %1
}
